...
### Wiring Diagram
![Wiring Diagram](./assets/wiring_diagram_v2.png "Wiring Diagram")
...
![Enclosure Image](./assets/BRUH Multisensor V1.PNG "Enclosure")

### Auto-Generating Your Arduino Project

If you have many sensors to make, consider using the automated project
generation python script.

* First install the requirements -- `$ pip install -r requirements.txt`
* Make a file called `.env` with your "secrets" (see
  `./examples_files/.env.example`)
* Source the file, so the variables are in your environment -- `$ source .env`
* Run the script -- `$ bin/generate <your_sensor_name>`

This will generate an Arduino project of the form:

```
bruh_mqtt_multisensor
└── bruh_mqtt_multisensor.ino
```

In addition, the script will print an auto-formatted yaml string to add to your
Home Assistant config.

### Manually Generating Your Arduino Project
If the above instructions seem too complicated, or you just need to make one
sensor, you can simply copy the text from the template file in
`./templates/bruh_mqtt_multisensor.j2` and put it into a new Arduino project.
Simply replace all `{{ }}` sections with the relevant information.
